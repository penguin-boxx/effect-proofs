...
Inductive lt : Type :=
  | lt_local : lt
  | lt_min : vars -> ListSet.set nat -> lt.

Definition lt_min_lts (L1 : lt) (L2 : lt) : lt :=
  match L1, L2 with
  | lt_local, _ => lt_local
  | _, lt_local => lt_local
  | lt_min free1 bound1, lt_min free2 bound2 =>
      lt_min (free1 \u free2) (set_union Nat.eq_dec bound1 bound2)
  end.
...
Inductive typ : Type :=
  ...
  | typ_bvar   : nat -> typ
  | typ_fvar   : var -> typ
  | typ_arrow  : lt -> typ -> typ -> typ
  | typ_all    : typ -> typ -> typ
  | typ_all_lt : lt -> typ -> typ.
...
Fixpoint open_tt_rec (K : nat) (Target : typ) (T : typ) {struct T} : typ :=
  match T with
  | typ_top l           => typ_top l
  | typ_bvar J          => If K = J then Target else (typ_bvar J)
  | typ_fvar X          => typ_fvar X
  | typ_arrow l Arg Res => typ_arrow l (open_tt_rec K Target Arg) (open_tt_rec K Target Res)
  | typ_all Bound T     => typ_all (open_tt_rec K Target Bound) (open_tt_rec (S K) Target T)
  | typ_all_lt l T      => typ_all_lt l (open_tt_rec K Target T)
  end.
...
Fixpoint open_te_rec (K : nat) (Target : typ) (e : trm) {struct e} : trm :=
  match e with
  | trm_bvar i          => trm_bvar i
  | trm_fvar x          => trm_fvar x
  | trm_abs T body      => trm_abs  (open_tt_rec K Target T) (open_te_rec K Target body)
  | trm_app lhs rhs     => trm_app  (open_te_rec K Target lhs) (open_te_rec K Target rhs)
  | trm_tabs Bound body => trm_tabs (open_tt_rec K Target Bound) (open_te_rec (S K) Target body)
  | trm_tapp t T        => trm_tapp (open_te_rec K Target t) (open_tt_rec K Target T)
  | trm_labs bound body => trm_labs bound (open_te_rec K Target body)
  | trm_lapp lhs l      => trm_lapp (open_te_rec K Target lhs) l
  end.
...
Fixpoint open_ee_rec (k : nat) (Target : trm) (e : trm) {struct e} : trm :=
  match e with
  | trm_bvar i          => If k = i then Target else (trm_bvar i)
  | trm_fvar x          => trm_fvar x
  | trm_abs V body      => trm_abs V (open_ee_rec (S k) Target body)
  | trm_app lhs rhs     => trm_app (open_ee_rec k Target lhs) (open_ee_rec k Target rhs)
  | trm_tabs Bound body => trm_tabs Bound (open_ee_rec k Target body)
  | trm_tapp lhs rhs    => trm_tapp (open_ee_rec k Target lhs) rhs
  | trm_labs l body     => trm_labs l (open_ee_rec k Target body)
  | trm_lapp lhs l      => trm_lapp (open_ee_rec k Target lhs) l
  end.

Definition open_ee t u := open_ee_rec 0 u t.

(** Opening up a lifetime binder ocurring in a lifetime *)

Definition open_ll_rec (k : nat) (Target : lt) (L : lt) : lt :=
  match L with
  | lt_local          => lt_local
  | lt_min free bound => if set_mem Nat.eq_dec k bound then
      let L' := lt_min free (set_remove Nat.eq_dec k bound) in lt_min_lts L' Target else L
  end.

Definition open_ll L l := open_ll_rec 0 l L.

(** Opening up a lifetime binder ocurring in a type *)

Fixpoint open_lt_rec (k : nat) (l : lt) (T : typ) {struct T} : typ :=
  match T with
  | typ_top L         => typ_top (open_ll_rec k l L)
  | typ_bvar J        => typ_bvar J
  | typ_fvar X        => typ_fvar X
  | typ_arrow L T1 T2 => typ_arrow (open_ll_rec k l L) (open_lt_rec k l T1) (open_lt_rec k l T2)
  | typ_all T1 T2     => typ_all (open_lt_rec k l T1) (open_lt_rec k l T2)
  | typ_all_lt L T    => typ_all_lt (open_ll_rec k l L) (open_lt_rec k l T)
  end.

Definition open_lt T l := open_lt_rec 0 l T.

(** Opening up a lifetime binder in a term *)

Fixpoint open_le_rec (k : nat) (l : lt) (e : trm) {struct e} : trm :=
  match e with
  | trm_bvar i     => trm_bvar i
  | trm_fvar x     => trm_fvar x
  | trm_abs V e1   => trm_abs (open_lt_rec k l V) (open_le_rec k l e1)
  | trm_app e1 e2  => trm_app (open_le_rec k l e1) (open_le_rec k l e2)
  | trm_tabs V e1  => trm_tabs (open_lt_rec k l V) (open_le_rec k l e1)
  | trm_tapp e1 V  => trm_tapp (open_le_rec k l e1) (open_lt_rec k l V)
  | trm_labs l' e1 => trm_labs (open_ll_rec k l l') (open_le_rec (S k) l e1)
  | trm_lapp e1 l' => trm_lapp (open_le_rec k l e1) l'
  end.

Definition open_le t l := open_le_rec 0 l t.
...
Notation "T 'open_ll_var' l" := (open_ll T (lt_min \{l} (empty_set nat))) (at level 67).
Notation "t 'open_lt_var' l" := (open_lt t (lt_min \{l} (empty_set nat))) (at level 67).
Notation "t 'open_le_var' l" := (open_le t (lt_min \{l} (empty_set nat))) (at level 67).

(** Lifetimes are locally closed pre-lifetimes *)

Inductive lt_lc : lt -> Prop :=
  | lt_lc_local :
      lt_lc lt_local
  | lt_lc_min : forall free,
      lt_lc (lt_min free (empty_set nat)).

(** Type are locally closed pre-types *)
...
Inductive type : typ -> Prop :=
  ...
  | type_var : forall X,
      type (typ_fvar X)
  | type_arrow : forall lt T1 T2,
      type T1 ->
      type T2 ->
      type (typ_arrow lt T1 T2)
  | type_all : forall L Bound T,
      type Bound ->
      (forall X, X \notin L -> type (T open_tt_var X)) ->
      type (typ_all Bound T)
  | type_all_lt : forall L Bound T,
      lt_lc Bound ->
      (forall l, l \notin L -> type (T open_lt_var l)) ->
      type (typ_all_lt Bound T).
...
Inductive term : trm -> Prop :=
  ...
  | term_tapp : forall e1 V,
      ...
      type V ->
      term (trm_tapp e1 V)
  | term_labs : forall L Bound body,
      lt_lc Bound ->
      (forall l, l \notin L -> term (body open_le_var l)) ->
      term (trm_labs Bound body)
  | term_lapp : forall lhs l,
      term lhs ->
      lt_lc l ->
      term (trm_lapp lhs l).
