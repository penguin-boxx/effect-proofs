...
(*                                                                    *)
(* Two scope notes.  (a) None of these states is REACHABLE from a     *)
(* well-typed source program — necessarily so: the bundle is          *)
(* preserved along every trace (Soundness.v), so a reachable violator *)
(* cannot exist; necessity is about the bundle being minimal as a     *)
(* HYPOTHESIS on arbitrary runtime states.  (b) Minimality is at the  *)
(* three top-level conjuncts: the closedness halves inside them       *)
(* (rt_closed, marker_annots_closed) serve trace PRESERVATION, not    *)
(* progress, and their necessity is not claimed.                      *)
(* ================================================================== *)
