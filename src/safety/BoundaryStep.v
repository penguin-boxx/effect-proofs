...
(*                                                                    *)
(* Boundary.v phrases impermeability over `boundary_crossing`, a      *)
...
(* channel ch.  The impermeability theorems then read as guarantees   *)
...
(*       enters the clause)                                           *)
(*       EXEMPT BY DESIGN: it is `local` by construction — typed      *)
(*       `A -local-> T_R` (source_boundary_resumption_local below),   *)
(*       so the closure-lifetime discipline pins it inside the        *)
(*       clause; its non-escape is the capability-confinement story   *)
(*       (source_capability_never_exposed, Escape.v), not a noloc     *)
(*       check.                                                       *)
...
(*       EXEMPT BY DESIGN: H_Perform CONSUMES the delimiter, so the   *)
(*       clause's answer lands in the outer context without crossing  *)
(*       any boundary — a local T_R is allowed, and T_HandlerM        *)
...
(* Source-facing event form of the impermeability theorem: along any   *)
...
(* lt_local.  The closure lifetime lt_local is forced: it is exactly   *)
(* what pins the resumption inside the clause (channel 2 of the        *)
(* matrix).                                                            *)
