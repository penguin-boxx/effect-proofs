...
### Current status
- 2026-06-10 update 2: the false broad `handler_perform_preservation` has been repaired at the syntax/semantics boundary by threading the handler result type through `term_handle`, `term_cap`, `term_handler_m`, and `term_resume`. `Syntax.v`, `Substitution.v`, `Typing.v`, `Semantics.v`, `SubstitutionTheory.v`, and `Safety.v` compile, and `make -C /home/penguin/Projs/effects-research/proofs all` passes. The remaining `handler_perform_preservation` axiom is now narrowed to the sound same-annotation case used by `H_Perform`. This refactor exposed a new necessary invariant, isolated as `handler_progress_annotation_match`: `marker_ok` alone records that a marker is active, but not that a cap's stored result type equals the surrounding delimiter's result type. Proving progress without an axiom therefore needs a typed marker environment/invariant mapping markers to result types, or an equivalent source-generated-runtime invariant.
...
- 2026-06-10 update: `canonical_ctor` has been deleted, not merely bypassed. Its last use in `local_value_does_not_escape` was reformulated with the data-tag premise `ctx_lookup_eff Γ K = None`, then switched to the proven `canonical_ctor_data`. `make -C /home/penguin/Projs/effects-research/proofs all` passes with only existing unused-pattern warnings.
- `typing_implies_wf` is now proven (no longer an axiom). `Print Assumptions Safety.type_safety` reports 7 remaining safety-critical obligations, with `handler_perform_preservation` stated over the stored `n_β` field. The refactor removes the previous blocker for proving `typing_InsTy`, `subst_ty_in_tm_lemma`, `subst_tm_lemma`, and `handler_perform_preservation`.
- The constructor-canonical safety dependency is discharged. `T_Ctor` and `T_Match` now require `ctx_lookup_eff Γ result_tag = None`, making the public constructor result tag explicitly data-only. `Safety.canonical_ctor_data` proves the needed canonical form under that premise, and `progress` uses it instead of the broad `canonical_ctor` axiom.
- The effect-receiver canonical-form dependency is also discharged. `canonical_cap` is now a proven `Lemma` in `Safety.v` (the dual of `canonical_ctor_data`): under an `eval_ctx`, a value at an effect type `type_ctor E_tag Δ Ts` with `ctx_lookup_eff Γ E_tag = Some (n_α,n_β,sig,ret)` is a runtime capability `term_cap E_tag m n_β Ts op_body`. Its `T_Ctor` case is impossible because the data/effect disjointness premise gives `ctx_lookup_eff Γ result_tag = None` while the goal tag has `Some _`. To support the `T_Sub` step via `sub_ctor_inv` (which needs `E_tag <> any_tag`), `eval_ctx`'s `ec_eff` constructor was strengthened with `E <> any_tag`, and a new lemma `SubstitutionTheory.eval_ctx_no_eff_any : eval_ctx Γ -> ctx_lookup_eff Γ any_tag = None` was added. This is sound because no core proof ever *constructs* an `eval_ctx` (it is only consumed as a hypothesis).
...
- Remaining assumptions on `Safety.type_safety` (7 total): `handler_perform_preservation`, `marker_ok_preservation`, `SubstitutionTheory.ctor_lts_chain_bounded`, and the four de Bruijn substitution lemmas (`subst_tm_lemma`, `subst_ty_in_tm_lemma`, `subst_lt_in_tm_lemma`, `subst_list_lt_in_tm_lemma`). The broad `handler_perform_preservation` is false as stated; `CounterexampleHandlerPerform.v` compiles and derives `False` from it. Sound repair likely requires runtime handler/cap/resume terms to carry the handler result type, so the untyped `H_Perform` rule can enforce that the cap's hidden op-body return type matches the enclosing delimiter.
