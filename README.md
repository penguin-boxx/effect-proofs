...
| `step_boundary_accounting` / `boundary_event_unique` / `source_step_accounting`                   | `src/safety/BoundaryStep.v`        | **The matrix is complete**: every reduction step is a frame step (both endpoints share the context, so the delimiter spine above the contraction is unchanged), a fresh-delimiter allocation, or exactly one boundary event — the three classes are mutually exclusive and the event's channel and crossing value are a function of the state, computed by the certified classifier `classify`; on source runs the event arm additionally carries its guarded-channel typing. |
...
| `source_safety_suite`                                                                             | `src/safety/Guarantees.v`          | **The umbrella theorem**: one record bundling type safety, invariant preservation, both confinement forms, capability-free escapable results, guarded-channel safety, resumption locality — and the operational story: per-step boundary accounting with channel typing, the evaluator halting only on values, result uniqueness modulo markers, and evaluator completeness — from `eval_ctx`, `sourceb t = true`, and one typing derivation.                        |
...
| `head_step_deterministic` / `step_deterministic_modulo_markers` / `stepf_complete_modulo_markers` | `src/safety/Determinism.v`         | Head reduction is a partial function; any two steps from one state agree up to a marker bijection; the certified evaluator is complete for the step relation modulo the fresh-marker choice — so every evaluator verdict is certified (`stepf_classification`): value / step / escape / genuinely stuck (no value, no step, no escape decomposition), and on well-typed source states the stuck verdict is unreachable (`source_stepf_never_stuck`, Guarantees.v).   |
| `multi_step_deterministic_modulo_markers` / `value_unique_modulo_markers` / `stepf_run_complete_modulo_markers` | `src/safety/Determinism.v`         | **Evaluation is a partial function up to marker permutation**: alpha-equivalence is a simulation (`step_marker_alpha_simulation`), any two normal forms — in particular any two values — reached from one state agree up to a marker bijection, and the bounded driver `stepf_run` reaches every value the relation can reach (adequacy: sound by `stepf_run_sound`, complete modulo markers).                                                                     |
